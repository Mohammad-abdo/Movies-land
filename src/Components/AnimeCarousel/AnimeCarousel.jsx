...
const AnimeCarousel = () => {
    ...
    useEffect(() => {
        const getList = async () => {
            try {
                setLoading(true);
                const params = {};
                const response = await tmdbApi.searchAnime({ params });
    ...
    if (error) {
        return <div className="text-light">Error loading anime</div>;
    }

    return (
        <Carousel responsive={responsive}>
            {items.map((anime) => (
                <Link key={anime.id} to={`/tv/${anime.id}`}>
                    <div className="rounded__card shadow mx-1 my-2">
                        <div className="card">
                            <img 
                                src={ApiConfig.originalImage(anime.poster_path)} 
                            ...
                            <div className="card-body card__body">
                                ...
                                {anime.overview && (
                                    <div className="text-light over__card" style={{ fontSize: '17px' }}>
                                        {anime.overview.substring(0, 100)}...
                                    </div>
                                )}
                            </div>
                        </div>
